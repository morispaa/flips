program comp_test
  ...
  integer :: oa,ob,oc,ia,ib,ic, i, j, bs, odfac
  ...
  case(1)
     do i = bmin,bmax,bstep
        do j = sstart,send,sstep
           ...
           if (i==0) then
              if (prec==1) then
                 call test_run_c(j,1,loopcnt,use_files,t,odfac)
              else
                 call test_run_z(j,1,loopcnt,use_files,t,odfac)
              end if
              bs = 1
           else

              if (prec==1) then
                 call test_run_c(j,i,loopcnt,use_files,t,odfac)
              else
                 call test_run_z(j,i,loopcnt,use_files,t,odfac)
              end if
              bs = i
           end if
  ...
  case(2)
     do i = sstart,send,sstep
        do j = bmin,bmax,bstep
           ...
           if (j==0) then
              if (prec==1) then
                 call test_run_c(i,1,loopcnt,use_files,t,odfac)
              else
                 call test_run_z(i,1,loopcnt,use_files,t,odfac)
              end if
              bs = 1
           else

              if (prec==1) then
                 call test_run_c(i,j,loopcnt,use_files,t,odfac)
              else
                 call test_run_z(i,j,loopcnt,use_files,t,odfac)
              end if
              bs = j
           end if


           write(*,'(i6,i6,f15.8,f6.3)') i,bs,t
...
contains
  ...
  subroutine test_run_c(size,buffer,loops,files,time,odf)
    implicit none

    integer, intent(in) :: size, buffer, loops, odf
    logical :: files
    ...
    allocate(amat(size*size*odf),mvec(size*odf),aux1(size*size*odf),aux2(size*odf))

    time = 0.0D0

write(*,*) 'Data construction start'
...
write(*,*) 'Data construction completed'
...
write(*,*) 'FLIPS start!'
    ...
    do n = 1,loops
       ...
       ! Feed the data
       call flips_add(gs,size*odf,amat,mvec)
       ! Solve
       call flips_solve(gs)
...
write(*,*) 'FLIPS complete'
  ...
  subroutine test_run_z(size,buffer,loops,files,time,odf)
    implicit none
