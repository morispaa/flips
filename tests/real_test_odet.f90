program real_test
  ...
  ! This version also over determined problems
  ...
  integer :: sstart,send,sstep,bmin,bmax,bstep,loopcnt,order, prec,mem,&
       matmode, bsize, csize
  integer :: oa,ob,oc,ia,ib,ic, i, j, bs,h, odfac
  ...
  case(1)
     do i = bmin,bmax,bstep
        do j = sstart,send,sstep
           ...
           if (i==0) then
              if (prec==1) then
                 call test_run_s(j,1,bsize,csize,odfac,loopcnt,use_files,t,f)
              else
                 call test_run_d(j,1,bsize,csize,odfac,loopcnt,use_files,t,f)
              end if
              bs = 1
           else
           
              if (prec==1) then
                 call test_run_s(j,i,bsize,csize,odfac,loopcnt,use_files,t,f)
              else
                 call test_run_d(j,i,bsize,csize,odfac,loopcnt,use_files,t,f)
              end if
              bs = i
           end if
  ...
  case(2)
     do i = sstart,send,sstep
        do j = bmin,bmax,bstep
           ...
           if (j==0) then
              if (prec==1) then
                 call test_run_s(i,1,bsize,csize,odfac,loopcnt,use_files,t,f)
              else
                 call test_run_d(i,1,bsize,csize,odfac,loopcnt,use_files,t,f)
              end if
              bs = 1
           else

              if (prec==1) then
                 call test_run_s(i,j,bsize,csize,odfac,loopcnt,use_files,t,f)
              else
                 call test_run_d(i,j,bsize,csize,odfac,loopcnt,use_files,t,f)
              end if
              bs = j
           end if
...
contains
  ...
  subroutine test_run_s(size,buffer,bsize,csize,odfac,loops,files,time,flops)
    implicit none

    integer, intent(in) :: size, buffer, bsize, csize, loops,odfac
    logical :: files
    ...
    allocate(amat(size*odfac,size),mvec(size*odfac,1))
...
!write(*,*) 'Construct data'
    call construct_mat_s(size,bsize,csize,odfac,amat)
    call nrand(mvec)
!write(*,*) 'Data constructed'
  ...
  subroutine test_run_d(size,buffer,bsize,csize,odfac,loops,files,time,flops)
    implicit none
    ...
    ! Make data
    call construct_mat_d(size,bsize,csize,odfac,amat)
    call nrand(mvec)
...
subroutine construct_mat_s(size,bsize,csize,odfac,mat)
  implicit none

  integer, intent(in) :: size, bsize, csize, odfac
...
subroutine construct_mat_d(size,bsize,csize,odfac,mat)
  implicit none
