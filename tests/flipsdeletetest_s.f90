program deletetest
  ...
  real(sp), parameter :: limit = 2.9
  real(sp), parameter :: pi = 3.14159265358979

  real(sp), dimension(2048) :: riodata, xax, fit,tmpx,tmpr
  real(sp), dimension(2048,xlen) :: model,tmpm
  real(sp), dimension(xlen) :: ntmp, sol, tmpsol
  real(sp) :: khi
  ...
  ! Read riometer data
  open(123,FILE="rio_s.data",FORM='unformatted',access='stream')
  ...
  call flips_init(h,xlen,1,buffersize=50,common=20)
...
!!$     !currentProblemSize = currentProblemSize - idx
     ...
     deletion: do i = 1,currentProblemSize
        if (olmask(i)) then
           call flipsdelete_s(h,1,model(i,:),riodata(i))
        end if
     end do deletion
...
!!$     model = tmpm
  ...
  ! Write solution
  open(123,FILE="rio_sol.data",access='stream',form='unformatted')
