program deletetest
  ...
  real(dp), dimension(:), allocatable :: delmod,deld
...
!!$     !currentProblemSize = currentProblemSize - idx
     ...
     ! Construct deletion matrices
     allocate(delmod(idx*xlen),deld(idx))

     auxidx = 1
     do i = 1,currentProblemSize
        if (olmask(i)) then
           delmod(yind(auxidx,1,xlen):yind(auxidx,xlen,xlen)) = model(i,:)
           deld(auxidx) = riodata(i)
           auxidx = auxidx + 1
        end if
     end do
...
!write(*,*) delmod
     ...
     !   if (olmask(i)) then
           call flipsdelete_d(h,idx,delmod,deld)
     !   end if
     !end do deletion

                deallocate(delmod,deld)
