...
!!$
     ...
     fadd2: do i = 1,currentProblemSize
        call flips_add(h,1,model(i,:),(/ riodata(i) /))
     end do fadd2

     !call flips_add(h,currentProblemSize,model(1:currentProblemSize,:),riodata(1:currentProblemSize))
  ...
  ! Write solution
  open(123,FILE="rio_sol_nodel.data",access='stream',form='unformatted',status='replace')
